...
static int read_frame (int mode)
{
	...
	{
		printf("read_frame failed with VIDIOC_DQBUF!\n");
		return -1;
	}
...
static int camera_fmt_set(int subch, int angle)
{
	...
	fmt.fmt.pix.pixelformat 	= V4L2_PIX_FMT_SRGGB12;	//V4L2_PIX_FMT_YUV420;		//V4L2_PIX_FMT_YUV422P;//V4L2_PIX_FMT_NV12;//V4L2_PIX_FMT_YUYV;
	...
	else
	{
		...
		subch_fmt.pixelformat	= V4L2_PIX_FMT_SRGGB12;	// V4L2_PIX_FMT_YUV420; 		//V4L2_PIX_FMT_YUV422P;//V4L2_PIX_FMT_NV12;//V4L2_PIX_FMT_YUYV;
...
static int main_test(int sel, int mode)
{
	...
	printf("%s(%d): sel:%d, mode: %d\n", __func__, __LINE__, sel, mode);
	...
	{
		printf("Error: camera_fmt_set failed.\n");
		//return -1;
	}
	if (-1 ==req_frame_buffers())
	{
		printf("Error: req_frame_buffers failed.\n");
		//return -1;
	}
